...
<div class="wp-block-button <?php echo esc_attr( $qcls ); ?>"><a class="wp-block-button__link wp-element-button" href="<?php echo esc_url( audira_amazon_cart_url( $p['asin'], $qty ) ); ?>" target="_blank" rel="sponsored nofollow noopener">Add <?php echo esc_html( $qty ); ?> to cart</a></div>
<!-- /wp:button --><?php endforeach; ?></div>
<!-- /wp:buttons -->

<!-- wp:paragraph {"className":"aud-panel__note"} -->
<p class="aud-panel__note">Amazon opens with the item and quantity already in your cart — you review everything before paying. <a href="<?php echo esc_url( audira_amazon_product_url( $p['asin'] ) ); ?>" target="_blank" rel="sponsored nofollow noopener">View the full listing</a></p>
